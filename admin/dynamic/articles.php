...
$query = $pdo->prepare("SELECT articles.*, users.login AS EditorLogin FROM articles JOIN users ON articles.Editor = users.Id WHERE articles.Id=:Id;");
$query->execute(['Id' => $Id]);
...
if (isset($_SESSION["log-session"]) && isset($_SESSION['log-session-data'])): 
    if ($_SESSION['log-session-data']["Admin"]):
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
            ...
            $stmt = $pdo->prepare("UPDATE articles SET Name = ?, Short_desc = ?, Full_desc = ? WHERE Id = ?");
            if ($stmt->execute([$name, $short_desc, $full_desc, $id])) {
                $_SESSION["log-mess-s"] = "Запись обновлена";
                echo "<script>location.href = 'https://localhost/cult_conn/admin/article.php';</script>";
            } else {
