...
if (isset($_SESSION["log-session"]) && isset($_SESSION['log-session-data'])): 
    if ($_SESSION['log-session-data']["Admin"] || $_SESSION['log-session-data']["Editor"]):
        ...
        $query = $pdo->prepare("SELECT museums.*, users.login FROM museums JOIN users ON museums.editor = users.id WHERE museums.editor = ? ORDER BY museums.id;");
        $query->execute([$current_u_id]);
        $mesuems_query_result = $query->fetchAll(PDO::FETCH_ASSOC);
?>
    <div class="container mt-5">
        <h2 class="mb-4">Информация о моих музеях</h2>
        <div class="mb-3">
            <form method="post">
                <a class="btn btn-primary" href="/cult_conn/editor/dynamic/museums.php">Добавить запись</a>
            </form>
        </div>
                ...
                <?php else: ?>
                    <?php foreach ($mesuems_query_result as $museum) { ?>
                        <tr>
                            ...
                            <td><a href="/cult_conn/editor/dynamic/museums.php?id=<?= htmlspecialchars($museum["Id"])?>">Редактировать</a></td>
                        </tr>
                    <?php }
                endif; ?>
            </tbody>
