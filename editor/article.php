...
if (isset($_SESSION["log-session"]) && isset($_SESSION['log-session-data'])): 
    if ($_SESSION['log-session-data']["Admin"] || $_SESSION['log-session-data']["Editor"]):
        ...
        $query = $pdo->prepare("SELECT articles.*, users.login FROM articles JOIN users ON articles.editor = users.id WHERE articles.editor = ? ORDER BY articles.id");
        $query->execute([$current_u_id]);
        $articles_query_result = $query->fetchAll(PDO::FETCH_ASSOC);
?>
    <div class="container mt-5">
        <h2 class="mb-4">Информация о моих статьях</h2>
        <div class="mb-3">
            <form method="post">
                <a class="btn btn-primary" href="/cult_conn/editor/dynamic/articles.php">Добавить запись</a>
            </form>
        </div>
                ...
                <?php else: ?>
                    <?php foreach ($articles_query_result as $article) { ?>
                        <tr>
                            ...
                            <td><a href="/cult_conn/editor/dynamic/articles.php?id=<?= htmlspecialchars($article["Id"])?>">Редактировать</a></td>
                        </tr>
                    <?php }
                endif; ?>
            </tbody>
