...
$query = $pdo->prepare("SELECT museums.*, users.login AS EditorLogin FROM museums JOIN users ON museums.Editor = users.Id WHERE museums.Id=:Id AND museums.Editor =:Editor;");
    ...
    if ($_SESSION['log-session-data']["Admin"] || $_SESSION['log-session-data']["Editor"]):
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
            ...
            if ($stmt->execute()) {
                ...
                echo 'window.location.href = "http://localhost/cult_conn/editor/museum.php";';
                echo '</script>';
            } 
        }
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
            ...
            if ($stmt->execute([$name, $short_desc, $full_desc, $id])) {
                ...
                echo "<script>location.href = 'https://localhost/cult_conn/editor/museum.php';</script>";
            } else {
...
<div class="container mt-5">
        ...
        <?php elseif($museum_query_result == 0): ?>
            <title>Создать данные музея</title>
            <form method="post" enctype="multipart/form-data">
            <div class="row">
                <div class="form-container">
                    <div class="col-md-4 image-column">
