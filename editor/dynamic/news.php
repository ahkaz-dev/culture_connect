...
$query = $pdo->prepare("SELECT news.*, users.login AS EditorLogin FROM news JOIN users ON news.Editor = users.Id WHERE news.Id=:Id AND news.Editor =:Editor;");
    ...
    if ($_SESSION['log-session-data']["Admin"] || $_SESSION['log-session-data']["Editor"]):
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
            ...
            if ($stmt->execute()) {
                ...
                echo 'window.location.href = "http://localhost/cult_conn/editor/new.php";';
                echo '</script>';
            } 
        }
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
            ...
            if ($stmt->execute([$name, $short_desc, $full_desc, $footer_desc, $id])) {
                ...
                echo "<script>location.href = 'https://localhost/cult_conn/editor/new.php';</script>";
            } else {
