...
$query = $pdo->prepare("SELECT product.*, users.login AS EditorLogin FROM product JOIN users ON product.Editor = users.Id WHERE product.Id=:Id AND product.Editor =:Editor;");
    ...
    if ($_SESSION['log-session-data']["Admin"] || $_SESSION['log-session-data']["Editor"]):
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
            ...
            if ($stmt->execute()) {
                ...
                echo 'window.location.href = "http://localhost/cult_conn/editor/product.php";';
                echo '</script>';
            } 
        }
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
            ...
            if ($stmt->execute([$name, $short_desc, $full_desc, $price, $available,  $id])) {
                ...
                echo "<script>location.href = 'https://localhost/cult_conn/editor/product.php';</script>";
            } else {
